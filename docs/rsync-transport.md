...
| `use chroot` ist die Isolationsgrenze | Auf alpine:3.19 brach `use chroot = yes` jeden Transfer (exit 23, musl 1.2.4); ab alpine:3.20 behoben, das Image steht auf 3.22. Die App setzt heute `use chroot = yes`. **Im ausgelieferten Container scheitert es trotzdem** — an der Rechtegrenze, nicht an musl: siehe „use chroot" und Ticket `33eeb98c`. |
...
`rsyncd.conf` (Modulname und Secret stammen aus dem Pairing, hier
`pairdc168e7c4bee`):
...
- Der Wert wird **in beiden chroot-Modi relativ zur Modulwurzel** aufgelöst. Mit
  ...
  nicht). Der führende Slash ist also richtig und bleibt richtig, egal wie `33eeb98c`
  die chroot-Frage entscheidet.
...
sequenceDiagram
    ...
    Note over B: Modulname = "pair" + 12 Hex<br/>Secret = 32 Zeichen aus CSPRNG
...
`speedup` laut rsync: 172,39 bzw. 2.279.513. sha256 von Quelle und Ziel danach
identisch.
...
Schicht, die auch dann trägt, wenn chroot nicht greift — und im ausgelieferten Container
greift chroot derzeit nicht (siehe „use chroot", Ticket `33eeb98c`). Wer nur auf chroot
baut, hat in genau diesem Zustand gar keinen Schutz.
...
**Es funktioniert im ausgelieferten Container trotzdem noch nicht**, und zwar aus einem
anderen Grund als dem hier beschriebenen: der Container läuft als `appuser`, `chroot()`
verlangt `CAP_SYS_CHROOT`. Ein Tester hat das am ausgelieferten Image gemessen; der
Client sieht `@ERROR: chroot failed`, im Daemon-Log steht
`chroot("<pfad>") failed: Operation not permitted (1)`. Das ist **Ticket `33eeb98c`** und
läuft parallel. `mirror_log_file` erkennt beide Hälften und hebt sie als
Deployment-Fehler heraus, statt sie als Transferproblem durchgehen zu lassen.

Also ehrlich zusammengefasst: `use chroot = yes` wird gesetzt, wirkt aber erst mit den
Rechten aus `33eeb98c`. Bis dahin trägt allein `refuse options` — deshalb ist es dort
keine „zweite Schicht", sondern die einzige.
...
2. **Basis-Image auf mindestens alpine:3.20 anheben.** ✅ **erledigt** mit Ticket
   ...
   `use chroot = yes`. **Offen bleibt die Rechtefrage** (`CAP_SYS_CHROOT`, Ticket
   `33eeb98c`) — bis die geklärt ist, greift chroot im Container nicht.
